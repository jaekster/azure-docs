...
1. Configure client-side registry setting for tenant discovery on your domain-joined computers using a Group Policy Object (GPO).
1. If you're using Active Directory Federation Services (AD FS), you must also configure the client-side registry setting for tenant discovery on your AD FS server using a GPO.
...
### Configure client-side registry setting for tenant discovery

Use the following example to create a Group Policy Object (GPO) to deploy a registry setting configuring tenant discovery in the registry of your devices.

1. Open a Group Policy Management console and create a new Group Policy Object in your domain.
   1. Provide your newly created GPO a name (for example, ClientSideTenantDiscovery).
...
If your Azure AD is federated with AD FS, you first need to configure client-side tenant discovery using the instructions mentioned earlier by linking the GPO to your AD FS servers. The SCP object defines the source of authority for device objects. It can be on-premises or Azure AD. When client-side tenant discovery is configured for AD FS, the source for device objects is established as Azure AD.

> [!NOTE]
> If you failed to configure client-side tenant discovery on your AD FS servers, the source for device identities would be considered as on-premises. AD FS will then start deleting device objects from on-premises directory after the stipulated period defined in the AD FS Device Registration's attribute "MaximumInactiveDays". AD FS Device Registration objects can be found using the [Get-AdfsDeviceRegistration cmdlet](/powershell/module/adfs/get-adfsdeviceregistration).
...
> [!NOTE]
> If a SCP is not configured in AD, then you should follow the same approach as described to [Configure client-side registry setting for tenant discovery](#configure-client-side-registry-setting-for-tenant-discovery)) on your domain-joined computers using a Group Policy Object (GPO).
